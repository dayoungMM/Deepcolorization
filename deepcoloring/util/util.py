...
def add_color_patches_rand_gt_user(data,user_hint,opt,p=.125,num_points=None,use_avg=True,samp='normal'):
# Add random color points sampled from ground truth based on:
#   Number of points
#   - if num_points is 0, then sample from geometric distribution, drawn from probability p
#   - if num_points > 0, then sample that number of points
...
#   - otherwise, draw from U[0, 1] of image
    for i in user_hint:
        ...
        W = int(float(W))
        H = int(float(H))
        P = np.random.choice(opt.sample_Ps)
        # rgb=np.zeros_like(data['hint_B'])
        rgb = np.zeros(shape=(1,3,1,1))
        rgb[:,0,:,:] = float(R) / 255.0
        rgb[:,1,:,:] = float(G) / 255.0
        rgb[:,2,:,:] = float(B) / 255.0
        ...
        data['hint_B'][0,:,H:H+P,W:W+P] = ab_rs
        
        data['mask_B'][0,:,H:H+P,W:W+P] = 1
